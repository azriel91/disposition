...
impl EdgePathBuilderPass2Ortho {
    ...
    pub(in crate::taffy_to_svg_elements_mapper) fn build_spacer_edge_path(
        ...
        // Minimise turns: when consecutive spacers run laterally in one
        // direction, an interior spacer whose cross-axis coordinate dips back
        // toward the others (a "notch") can be passed at the neighbouring
        // turn's coordinate instead of forcing the path to jog in and back out.
    ...
    /// Reduces the number of turns in a multi-spacer orthogonal path by
    /// snapping "notch" spacers onto a neighbouring turn's cross-axis
    /// coordinate, so their routing legs merge instead of forcing an extra
    /// in-then-out jog.
    ///
    /// # The optimisation
    ///
...
mod tests {
    ...
    use super::EdgePathBuilderPass2Ortho;
    ...
    fn turn_minimize_skips_interior_notch_spacers() {
        ...
        let result = EdgePathBuilderPass2Ortho::spacers_turn_minimize(
            80.0,
            0.0,
            50.0,
            500.0,
