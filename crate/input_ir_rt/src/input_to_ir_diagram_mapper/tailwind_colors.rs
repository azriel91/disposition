//! Tailwind CSS color name and shade to oklch value mapping.
//!
//! This module provides a compile-time lookup table that maps Tailwind CSS
//! color names (e.g. `"slate"`, `"red"`) and shade values (e.g. `"500"`,
//! `"900"`) to their corresponding oklch color strings.
...
/// Looks up the oklch value for a given Tailwind CSS color name and shade.
///
/// # Parameters
///
/// * `color`: The Tailwind CSS color name, e.g. `"slate"`, `"red"`, `"blue"`.
/// * `shade`: The shade value, e.g. `"50"`, `"500"`, `"950"`.
///
/// # Returns
///
/// The oklch color string if the color and shade combination exists, e.g.
/// `"oklch(63.7% 0.237 25.331)"`, or `None` if not found.
pub(crate) fn tailwind_color_lookup(color: &str, shade: &str) -> Option<&'static str> {
    let tailwind_color = TAILWIND_COLORS
        .iter()
        .find(|tailwind_color| tailwind_color.name == color)?;
    tailwind_color
        .shades
        .iter()
        .find(|(shade_key, _oklch_value)| *shade_key == shade)
        .map(|(_shade_key, oklch_value)| *oklch_value)
}
...
mod tests {
    ...
    #[test]
    fn lookup_returns_correct_oklch_for_known_color_and_shade() {
        assert_eq!(
            tailwind_color_lookup("slate", "500"),
            Some("oklch(55.4% 0.046 257.417)")
        );
    }

    #[test]
    fn lookup_returns_correct_oklch_for_edge_shades() {
        assert_eq!(
            tailwind_color_lookup("red", "50"),
            Some("oklch(97.1% 0.013 17.38)")
        );
        assert_eq!(
            tailwind_color_lookup("red", "950"),
            Some("oklch(25.8% 0.092 26.042)")
        );
    }

    #[test]
    fn lookup_returns_none_for_unknown_color() {
        assert_eq!(tailwind_color_lookup("nonexistent", "500"), None);
    }

    #[test]
    fn lookup_returns_none_for_unknown_shade() {
        assert_eq!(tailwind_color_lookup("slate", "999"), None);
    }

    #[test]
    fn lookup_returns_none_for_excluded_colors() {
        assert_eq!(tailwind_color_lookup("black", "500"), None);
        assert_eq!(tailwind_color_lookup("white", "500"), None);
        assert_eq!(tailwind_color_lookup("transparent", "500"), None);
    }

    #[test]
    fn all_colors_have_eleven_shades() {
        for tailwind_color in TAILWIND_COLORS {
            assert_eq!(
                tailwind_color.shades.len(),
                11,
                "color `{}` should have 11 shades (50, 100..900, 950)",
                tailwind_color.name
            );
        }
    }
    ...
    #[test]
    fn tailwind_colors_count() {
        assert_eq!(TAILWIND_COLORS.len(), 26);
    }
}
