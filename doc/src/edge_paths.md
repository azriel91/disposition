...
5. **Gap-transit separation** (`fn face_offsets_gap_transit_separate`): the steps above only consider edges that **contact** a node face. But an edge entering a **container** `C` at face `F` can run near-parallel to another edge that merely **transits** the inter-rank gap just before `C` on its way to a node nested *inside* `C` -- the transiting edge never contacts `C`, so the offset machinery cannot see it. In `0037` (`left_to_right`), `edge_dep_b_0_c_0` enters `t_c_0`'s `Left` face at y = 99.375 while `edge_dep_a_01_c_01` (routing to `t_c_01` nested in `t_c_0`) transits the same gap at y = 100.5; the two legs sit ~1 px apart and read as one line.

    This post-pass runs after the cross-node separation, on `face_offsets_by_node_face`. For each container to-face contact, it finds edges whose to-node is a **strict descendant** of the container (`fn node_is_descendant_of`) and resolves the cross-axis coordinate of the descendant edge's spacer that transits the gap **just before** that face (`fn transit_cross_axis_before_face`, picking the nearest spacer on the approach side -- robust to `RankDir`). If the container contact lands within `CONTACT_GAP_MIN_PX` of a transit coordinate, the contact is shifted along the face axis to clear it by a full gap, staying within the face (`fn contact_offset_nudged_from_transits`). Only container faces with a transiting descendant edge inside the contact gap are adjusted, so other layouts are unchanged. For `0037` the contact moves from y = 99.375 to y = 88.5, ~12 px clear of the transit.
