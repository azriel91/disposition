...
Offset calculation places each edge's contact point along a node face at a position that either aligns with the edge's description label or, when no label is present, spreads contacts evenly so they do not overlap. The offset for each edge is a signed pixel distance from the face midpoint. Offsets are computed in `fn face_offsets_compute` in [`svg_edge_infos_builder.rs`](crate/input_ir_rt/src/taffy_to_svg_elements_mapper/svg_edge_infos_builder.rs), with label spans resolved via `SvgEdgeInfosBuilder::label_face_span_compute` / `SvgEdgeInfosBuilder::taffy_node_absolute_xy_compute`, and slot-based fallback arithmetic provided by [`edge_face_contact_tracker.rs`](crate/input_ir_rt/src/taffy_to_svg_elements_mapper/edge_face_contact_tracker.rs).
...
- **Label-based offset**: when an edge has a non-zero description label node on a face (looked up from `TaffyNodeMappings::edge_label_taffy_nodes`), the contact point is routed to the entry-side edge of the label -- the side the path arrives at first. Because sibling insertion order is reversed for reversed rank directions (see [Sibling order for reversed rank directions](#sibling-order-for-reversed-rank-directions)), the entry side is the same for all rank directions:
  ...
  - `SvgEdgeInfosBuilder::label_face_span_compute` resolves not just this entry-side offset but the label's **full occupied span** (`LabelFaceSpan { near, far }`, both already adjusted by the halo pullback/push described above) -- the far edge is used by the slot-based fallback below to steer other edges clear of real label boxes.

- **~~Bidirectional pair offset~~ (removed)**: `kind: symmetric` interactions/dependencies produce a forward (`EdgeType::PairRequest`) and reverse (`EdgeType::PairResponse`) edge pair. `EdgePathBuilderPass2::build` used to additionally shift each unlabeled endpoint by `node.width` (or `height_collapsed` for `Left`/`Right` faces) `* BIDIRECTIONAL_OFFSET_RATIO (0.1)`, in a direction fixed by `PairRequest`/`PairResponse`, so the two arrows of an unlabeled pair wouldn't land on the exact same face-midpoint line. This is now redundant: `face_offsets_compute` already registers each direction of a symmetric pair as its own `FaceContactEntry` and spreads every contact on a face -- label-based or slot-fallback -- via `EdgeFaceContactTracker::offset_for_index`, so both directions are separated before pass 2 ever runs. Applying the shift on top double-counted that separation and could push one pair member's contact onto an unrelated edge's already-correct slot (e.g. a same-face self-loop pair colliding with the next sequence edge's contact). It also made the `edge-routing` CLI diagnostic (which reads `face_offsets_by_node_face` directly, before this pass-2-only shift) silently stale for any `Pair*` edge whose contact wasn't label-based. The block, `BIDIRECTIONAL_OFFSET_RATIO`, and the `EdgeFaceOffset`/`EdgeContactPointOffsets` `is_label_based`/`from_offset_is_label`/`to_offset_is_label` plumbing that only existed to gate it have all been deleted.

- **Face contact clamp**: after the label/slot offset is applied, `EdgePathBuilderPass1::face_contact_clamp` clamps the contact point to the node's own face span (leaving `FACE_CONTACT_MARGIN_PX` on each side), as a defensive backstop against any offset source placing the contact outside the node's own rectangle. This runs before the curvature-specific branch, so it applies to `Orthogonal`, `Curved`, and `Direct*` edges alike.
...
- **Fallback clearance from real label geometry** (`fn face_offsets_slot_offset_labels_clear`): the per-kind slot arithmetic above has no awareness of where any edge's *real* label rendered, including a co-located edge from the *other* kind pool sharing the same face -- e.g. `edge_dep_client_server__1` (no label, dependency-pool fallback) and `edge_ix_client_server__0` (real `to_label`, interaction pool) in `0057_interaction_halo_with_desc_cyclic`, both touching the same `t_server` face. Before this, the fallback's naive centered offset could land inside the other edge's real, text-bearing label box. `face_offsets_compute` now resolves every entry's `label_face_span_compute` result up front (regardless of kind) into `label_spans_occupied`, and passes it to `face_offsets_slot_offset_labels_clear`, which nudges a slot-based offset to the nearer edge of any span it falls inside (plus `CONTACT_GAP_MIN_PX` clearance). A face with no real labels at all -- e.g. every edge in `0044_edge_offsets_and_protrusion_complex_2` -- has an empty span list, so this is a no-op and 0044's centered/coincident fallback behaviour (co-located label-less pairs sharing the exact face-midpoint contact) is unchanged.

- **Label-cleared collision separation** (`fn face_offsets_label_cleared_collisions_separate`): the clearing step above nudges one entry against one span at a time, so two *different* fallback entries -- possibly from different kind pools -- can each be nudged to the same span's boundary (identical offsets), or to the boundaries of two different but adjacent real label spans (offsets that end up only a pixel or two apart). Nothing about the clearing step itself coordinates between entries. `face_offsets_compute` tracks a parallel `was_label_cleared` flag (`cleared_offset != slot_offset`) and, after `face_offsets_self_loop_separation_enforce`, runs this pass: sort all contacts on the face by offset, split into runs of consecutive `was_label_cleared` entries bounded by fixed anchors (real labels *and* untouched fallback offsets, e.g. 0044's intentional same-kind-pool `0.0` coincidence), and redistribute each run around its own centroid with at least `CONTACT_GAP_MIN_PX` between neighbours and bounds. A face with no label-cleared entries at all is a structural no-op.
...
3. **Compute offset values.** For each contact entry, the offset is chosen as follows:
    - **Label-based** (preferred): if the edge has a non-zero label node on this face (looked up from `edge_label_taffy_nodes` via `label_face_span_compute`), the offset is the span's `near` value, i.e. `label_contact_on_face_axis - face_midpoint_on_face_axis`, where `label_contact` is the entry-side edge of the label (left x for `Top`/`Bottom` faces; top y for `Left`/`Right` faces). The absolute position is computed via `taffy_node_absolute_xy_compute`.
    - **Slot-based fallback**: when the edge has no label content on this face, `EdgeFaceContactTracker::offset_for_index` is used: `offset[i] = (i - (n - 1) / 2.0) * gap`, where `i` is the entry's index **within its kind's sub-list** and `n` is that kind's contact count. This distributes each kind's offsets symmetrically around 0 (the face midpoint). The first slot gets the most negative offset (leftward/upward), the middle slot(s) get ~0, and the last slot gets the most positive offset (rightward/downward). The result is then passed through `face_offsets_slot_offset_labels_clear` (see **Fallback clearance from real label geometry** above) to nudge it clear of any real label span on the same face.
    ...
    - **Label-cleared collision separation** (`fn face_offsets_label_cleared_collisions_separate`, see above): runs last, re-separating any fallback offsets that independently converged after label-clearing.
...
25. **Step 3: Assign protrusion depths (`fn protrusions_assign`).**
    ...
    **Jog separation (`fn jogs_separate`).** The proportional band split (below) sizes each side's band from the *tightest* `rank_gap_px` in the whole bucket. Because cross-container edges are lifted to their lowest-common-ancestor (LCA) rank gap, a fan of such edges sharing one LCA gap is bucketed together with unrelated short-gap endpoints, so their lateral legs collapse onto (near-)identical depths and read as one line (e.g. `edge_dep_ranks_slots` and `edge_dep_labels_offsets` in `018_edge_offsets_and_protrusions`). After the band split, `jogs_separate` re-nests **only** the spacer legs whose cross-axis spans (`[cross_axis_coord, jog_far_cross_axis]`) actually overlap and that have either **collapsed** (within `JOG_SEPARATION_MIN_PX = 7` of each other) or are **misordered** relative to the nesting rule below -- legs whose spans are disjoint may share a depth, since their legs never coincide (interval-graph style).

    The nesting rule is stated in terms of each leg's **descent column** -- the cross-axis coordinate of the spacer / node the path continues along *after* the lateral jog (for a spacer entry or to-endpoint that is the endpoint's own `cross_axis_coord`; for a from-endpoint or spacer exit it is the next contact's `jog_far_cross_axis`). A leg **sweeps over** another when that other leg's descent column lies strictly inside its span, so the sweeping leg's lateral segment passes over the column the other descends along; the two must then sit at distinct depths or they cross. Each leg's jog sits at `boundary +/- depth` (`+` on the `Low` side, `-` on the `High` side), so the same physical "the sweeping leg clears the swept leg's descent" maps to **opposite** depth orders per side: on the `High` side a leg that sweeps over more must be **deeper**; on the `Low` side, **shallower**. The order is a **topological sort** of the `sweeps_over` relation, not a raw sweep *count*: each leg's `layer` is the longest `sweeps_over` chain below it (`layer[i] = 1 + max(layer[j])` over the legs `i` sweeps), so `A` sweeps `B` always implies `layer[A] > layer[B]` even when `B` sweeps more unrelated legs in total (a count-based order would invert that pair). The relation can rarely be cyclic (two legs each containing the other's descent column), so the longest-path relaxation is capped at `n` passes. The sign is folded into the layer key by side, and legs are re-laid deepest-first in that order, each forced at least `JOG_SEPARATION_MIN_PX` shallower than the overlapping legs already placed deeper than it (so the order holds even when the legs' channel widths are not monotonic with it) and never past its same-edge connecting partner on the opposite side (`min(rank_gap_px) * MAX_GAP_FRACTION - partner_depth`), so the deepened leg cannot overshoot its partner and reverse the path. This keeps a fan of overlapping legs nested so no lateral sweep cuts across another's descent -- both a right-sweeping fan (the top-gap fan in `0044_edge_offsets_and_protrusion_complex_2`, and `edge_dep_ranks_slots` vs `edge_dep_labels_offsets` in `0043_edge_offsets_and_protrusion_complex_1`) and a left-sweeping fan (the label-band return jogs in `0044`). Within an equal layer the wider channel claims the deeper depth.

    Two further rules keep crowded LCA buckets from collapsing the shallow end of a chain: (1) the re-laid spacer legs floor at `MIN_PROTRUSION_PX` (a spacer has no arrow head), **not** the side band floor, which on a side holding to-endpoints is the larger `TO_PROTRUSION_MIN_PX` arrow-head clearance; and (2) only **spacer** legs act as fixed obstacles -- a from / to endpoint shares the same LCA bucket but sits at an unrelated rank-axis position (a deeply-nested to-endpoint is registered here yet physically descends far inside the container, and its placeholder depth is overwritten by Steps 5 / 5.5 / 5.6), so it is excluded as an obstacle. Without these the `0044` top-gap fan pins `edge_dep_ir_pass1` and `edge_dep_layout_contacts` onto the floor and they read as one line. From / to endpoints are themselves left fixed (their approach legs are separated by Steps 5.5 / 5.6). Buckets that are already nested and separated are byte-for-byte unchanged.

    *Known limitation:* `jogs_separate` nests legs by protrusion **depth** (distance from each leg's own spacer boundary), which equals visual separation only when the legs protrude from the **same** boundary row. When two edges' spacers sit on different rows -- e.g. cross-container spacers padded differently per row -- equal depth separation does not guarantee equal *position* separation. A position-aware (absolute-coordinate) nesting would be needed to fully guarantee no overlap in those cases.
    ...
    *Known limitation (label-routing return jogs):* a text-content spacer that routes an edge around a described node's label sits at the label's **measured** row bottom, which can fall a few pixels **above** the rendered text bottom (the text leaf measurement slightly underestimates the rendered line height). The nesting rule above places the innermost label-routing edge's return jog at the shallowest depth, i.e. right at the spacer's exit, so that jog can graze the last text line by those few pixels. Protrusion adjustment cannot push it lower without overshooting the next (target-column) spacer -- which itself sits at the label level -- so a clean fix belongs in the text-row height measurement / text-content spacer placement, not the protrusion calculator.
...
46. **Cross-container column snap.** Before merging, an edge's cross-container spacers are collapsed onto a single straight **cross-axis column** by `fn spacers_snap_to_outermost_column` in [`spacer_coordinates_resolver.rs`](crate/input_ir_rt/src/taffy_to_svg_elements_mapper/spacer_coordinates_resolver.rs). Cross-container spacers are *appended* to each rank row, so their cross-axis position is set by how much sibling content precedes them in that row. When an edge routes through several rows whose preceding content differs -- e.g. a deeper row drops a sibling edge's spacer that had padded this edge's spacer outward in the shallower rows -- the per-row spacers land at different cross-axis coordinates and the path zig-zags, which can cross a neighbouring straight-column edge (`edge_dep_ranks_slots` vs `edge_dep_labels_offsets` in `0043`). All of an edge's cross-container spacers sit on the **same** side of the rows' nodes (the gap side they were appended on), so collapsing them onto the single **outermost** coordinate keeps the column clear of every row's node -- each row's node ends at or inside its own spacer. The outermost coordinate is the **maximum** cross-axis value in every `RankDir` (nodes pack toward the low-coordinate side of a rank row and appended spacers land at the high-coordinate side, including for the reversed-flex `BottomToTop` / `RightToLeft` flows -- see the function doc comment for why an earlier min-based version for reversed flows was wrong). Because the snap acts only on the resolved coordinates shared by both the path builder and the protrusion calculator, both agree on the straightened column. A single cross-container spacer is already a straight column, so the snap is a no-op below two.

47. **Text-content (node-label) spacers ride along when they sit past the column.** Spacers in `EdgeSpacerTaffyNodes::text_content_spacer_taffy_node_ids` (see [Edge Spacers -- Text-Content (Node-Label) Spacers](edge_spacers.md)) are resolved as routing waypoints and are considered by `spacers_snap_to_outermost_column`, but they are not blanket-merged into the column: only a text-content spacer that sits **further along the path** than the (already two-or-more-spacer) cross-container column, and whose own position is *less* extreme than that column, is pulled out to meet it. Left alone, such a spacer would sit inside a column the edge already committed to, so the path would dip back in to reach it and then jog back out again toward the to-node -- an unnecessary extra Z-bend (e.g. `t_aws_az1_subnet_tier_private`'s label spacer sitting inside the wider `t_aws_az1` cross-container column in `0062_edges_from_higher_rank_to_lower_rank`). A text-content spacer that sits **earlier** than the column -- i.e. it belongs to the same, outer container whose siblings produced the column -- is left at its own local position: bowing out from the from-node to that spacer, then further out again to the column, is a single smooth outward bend, not a dip, and snapping it early would only misalign it from other edges sharing its rank gap (e.g. `edge_dep_layout_contacts` vs `edge_dep_ir_pass1` in `0044`, whose single, unsnapped cross-container and text-content spacers must stay at their own positions for `jogs_separate` to keep their return jogs separated). A text spacer that is already further out than the column (a deliberate detour past the edge's own rank column, e.g. `labels_offsets` in `0044`) is likewise left alone, since there is no dip to fix. To keep the **return jogs** of multiple edges (label column back to each edge's rank column) at distinct depths, the node's text `Row` carries a bottom margin of `N * TEXT_CONTENT_SPACER_GAP_PX`, enlarging only the label -> rank-0 gap so the protrusion band can stagger one jog depth per edge (`0044_edge_offsets_and_protrusion_complex_2`).
