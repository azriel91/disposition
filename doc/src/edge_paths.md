...
4. **Cross-node coincident-contact separation** (`fn face_offsets_collisions_separate`): the per-`(node, face)` slot logic above only spreads contacts that share a single node face. Two (or more) edges that exit the **same face direction of different nodes** can still resolve to the **same absolute coordinate** -- most commonly an edge from a container and an edge from a node nested and **centered** within it (taffy centers a single child, so the container and child share a face midpoint), e.g. in `0036` the `Bottom`-face edges from `t_a_0`, `t_a_00`, and `t_a_01` all land at x = 71. Because they are in different `(node, face)` groups, the slot logic never sees them together, so their protrusion stubs overlap.
    ...
    This post-pass runs after the per-face loop, on the final `face_offsets_by_node_face` map. It flattens every contact into a `FaceContactCollisionRecord` carrying the contact's **absolute** coordinate (`face_midpoint + offset`), groups records by exact `NodeFace`, sorts by absolute coordinate, and clusters adjacent records whose coordinate gap is within `CONTACT_GAP_MIN_PX`. A cluster is only adjusted when it spans **two or more distinct `(node, face)` groups** (a cluster wholly within one group is already spread, and a single-record cluster needs no separation -- so layouts without cross-node coincidence are byte-for-byte unchanged). Each collision cluster of `n` records is redistributed **symmetrically around the cluster's shared midpoint** (`center = mean of absolute coordinates`), with a gap of `EdgeFaceContactTracker::gap_calculate(n, min_face_length)` (sized from the narrowest node's face so the fan fits within all of them). Records are ordered deterministically by `(rank_distance asc, pass1_group_index asc, edge_index asc)` so closer-ranked edges sit innermost, and the new offset (`abs_pos - midpoint`) is written back per slot via `EdgeContactPointOffsets::offset_set`. For `0036` the three contacts spread from a shared x = 71 to x = 59 / 71 / 83.
...
17. **Rank gap**: the free span between two consecutive contact coordinates along the rank (main) axis -- e.g. a from-node face and the first spacer entry, two consecutive spacer boundaries, or a last spacer exit and a to-node face. Every edge endpoint that exits or enters a node face protrudes into a rank gap. Every spacer entry/exit boundary also protrudes into a rank gap.
18. **Rank gap key**: a `(lo, hi)` pair of **physical** main-axis boundary coordinates (quantized to a fixed grid by `OrthoProtrusionCalculator::quantize`) identifying a specific gap. All endpoints (from-node, to-node, spacer entry, spacer exit) whose legs occupy the same physical band are grouped together by this key. Implemented as the `RankGapKey` struct. Physical coordinates -- rather than LCA-level ranks -- are essential for **cross-container edges**: a cross-container edge's LCA gap collapses several physical sub-rank gaps into one coarse rank pair, so keying by LCA ranks lumps physically-distant endpoints (and the from-endpoints of sibling cross-container edges) into one bucket, defeating the per-gap staggering. Because taffy lays sibling rank rows -- and the spacers within them -- at identical coordinates, the from-endpoints of sibling cross-container edges naturally share one physical gap key and so compete for distinct depths.
19. **Gap side**: each endpoint in a rank gap protrudes from either the `Low` side (the `lo` boundary) or the `High` side (the `hi` boundary), determined by which boundary the endpoint's own main-axis coordinate matches. This is direction-agnostic (it uses coordinates, not rank arithmetic), so all four `RankDir`s and backward edges are handled uniformly. For example, a from-node protrudes from the `Low` side of its first gap toward the first spacer entry on the `High` side; the to-node protrudes from the `High` side of its last gap.
...
24. **Step 2: Collect rank gap entries.**
    ...
    - **Same-rank (cycle) edges** with `Top` or `Bottom` faces are handled by `cycle_edge_collect_rank_gap_entries`. A single entry is registered in the adjacent rank gap, keyed by the **physical** `(lo, hi)` of the from-face main-axis coordinate and the adjacent rank boundary: `Top` face protrudes from the `High` side (the face), `Bottom` from the `Low` side. The `rank_gap_px` for the endpoint is the pixel distance from the node's face to the nearest boundary of the adjacent rank (the maximum bottom edge of rank-R-1 nodes for `Top`, or the minimum top edge of rank-R+1 nodes for `Bottom`). **Only nodes of the same category as the from-node** are included in this boundary search, so thing-node cycle edges are not pushed out by process nodes at the same rank. Cycle edges with `Left` or `Right` faces are skipped here and fall through to the `MIN_PROTRUSION_PX` safety net in Step 6.
    - **Non-cycle edges** are handled by `non_cycle_edge_collect_rank_gap_entries`, which walks the edge's contact coordinates along the main axis and registers each **physical gap** (a pair of bounding endpoints) via `gap_pair_register`: from-node face -> first spacer entry, each spacer exit -> the next spacer entry, and the last spacer exit -> to-node face (or from-face -> to-face with no spacers). Because a gap's two sides come from two different endpoint kinds, each edge contributes at most one entry per side per bucket.
    - Each entry records its `GapSide` (derived from which physical boundary it protrudes from), cross-axis coordinate, face offset, and `rank_gap_px` (the gap span `hi - lo` minus that endpoint's own envelope clearance, re-added in `protrusion_write`).
...
27. **Step 5: Enforce minimum protrusions to clear divergent ancestor siblings (`fn protrusions_adjust_for_divergent_siblings`).**
    ...
    *Note on spacer-crossing fan-ins:* two spacer-crossing edges that enter the **same to-node** via spacers exiting at the same coordinate (e.g. `0030`) used to need a dedicated pass to separate their approach legs. With physical-gap bucketing their shared last-gap channel (last spacer exit -> to-face) now lands in **one bucket**, so `protrusions_assign` separates them directly -- no special-casing required.

27a. **Step 5.5: Nest approach legs of edges entering the same to-face from different rank-gap buckets (`fn protrusions_separate_shared_to_face_channels`).**

    Physical-gap bucketing groups by the gap's full `(lo, hi)` span. Two edges entering the **same to-face** from gaps of different lengths -- a plain edge into a nested node (its gap bounded below by a same-container sibling) and a cross-container spacer edge (its last gap bounded below by its last spacer) -- share the `hi` boundary (the to-face) but differ in `lo`, so they land in **different buckets** and never compete in `protrusions_assign`. Their approach legs are then chosen independently and can cross. In `0036`, the local edge `t_c_00 -> t_c_01` and the cross-container edge `t_a_01 -> t_c_01` both enter `t_c_01`'s `Top` face; the cross-container edge's deeper leg sweeps across the local edge's vertical leg and then its horizontal leg -- two crossings.

    This step groups non-cycle to-endpoints by `(to-node id, to-face)`. For each group that **mixes** a spacer-crossing edge with at least one other edge (the cross-bucket case; pure single-bucket fan-ins stay with `protrusions_assign`), it nests the legs within the **physical band** between the to-face and the nearest same-container, same-category sibling on the approach side (`fn approach_band`, robust to `RankDir` because it selects the nearest sibling in the to-face's outward direction rather than by rank arithmetic). Depths are stacked from the to-side floor (`TO_PROTRUSION_MIN_PX` / own-label clearance) upward in `MIN_PROTRUSION_PX` steps, deepest for the smallest face offset (matching the deepest-first-by-offset convention in `protrusions_assign`), capped at `MAX_GAP_FRACTION` of the band. For spacer-crossing edges the last spacer's `exit_protrusion` is updated so the spacer exit and the to tip still meet on the chosen leg. Groups without the mix are left unchanged, so single-bucket layouts are byte-for-byte unchanged.
...
33. **`non_cycle_edge_collect_rank_gap_entries`**: registers a non-cycle edge's from/to/spacer endpoints, bucketed by the physical main-axis gap each leg occupies. Builds the from/to/spacer endpoint descriptors (`GapEndpoint`) and walks consecutive pairs to `gap_pair_register`. Because the gap span `hi - lo` is shared by both sides of the bucket, `min_gap_px` and `available` in `protrusions_assign` become physically meaningful.
34. **`gap_pair_register`**: registers the two endpoints bounding one physical gap under a `RankGapKey` of the quantized `(lo, hi)` boundaries, assigning each endpoint its `GapSide` (Low when it protrudes from `lo`, High from `hi`) and `rank_gap_px = (hi - lo) - own envelope clearance`.
35. **`quantize`** / **`node_face_main_coord`**: `quantize` rounds a main-axis coordinate to a 0.25 px grid so legs in the same physical band hash to the same key; `node_face_main_coord` returns a node face center's main-axis coordinate (rank-axis), falling back to the pass-1 node coordinates.
...
38. **`main_axis_coord`**: returns the main-axis (rank-axis) coordinate of a point: Y for `Top`/`Bottom` faces, X for `Left`/`Right` faces. The complement of `cross_axis_coord`; used to derive physical gap keys.
...
43. For edges between deeply-nested nodes the band-assigned `from_protrusion` can exceed the available gap between the two outer containers. When the divergent-sibling adjustment (Step 5) simultaneously raises `to_protrusion` to clear the destination container hierarchy, the sum `from_protrusion + to_protrusion` can exceed the full node-to-node gap. This places the from-tip *past* the to-tip in the routing direction -- the tips cross -- so any Z/S segment drawn between them would re-enter the destination container.
    ...
    Note: within a single physical rank gap the per-side proportional split (Step 31) already guarantees `from_protrusion + to_protrusion <= MAX_GAP_FRACTION * gap`, and the gap span (`hi - lo`) is the actual inter-rank gap. The `from_protrusion_capped` guard is a rarely-hit secondary safety net for remaining edge cases (e.g. very small rank gaps, or the divergent-sibling adjustment raising `to_protrusion` after band assignment).
...
45. After `from_protrusion_capped` lands both protrusion tips at the same Y (or X for horizontal routing), the two tip waypoints have **opposite** departure directions. Since the path runs from-node to to-node, the segment is traversed from the from-tip to the to-tip: the from-tip departs downward (`Bottom` face, `dir = (0,+1)`) and the to-tip departs upward (`Top` face, `dir = (0,-1)`). The standard Z/S U-bend logic would route:
    ...
    This guard is rarely triggered, since the per-side proportional band split keeps both protrusion tips from landing at the same coordinate in normal routing. It remains as a safety net for degenerate layouts.
