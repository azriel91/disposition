...
48. **Spacer turn minimization.** The column snap (item 46) only unifies spacers of the *same* kind (cross-container spacers among themselves); a text-content spacer excluded from that snap (item 47), or any other spacer whose own local position happens to sit strictly between two other spacers' turns, can still leave a "notch" -- the path bows out to a wider column, dips back in to visit the notch spacer, then bows out again, an unnecessary extra bend. `EdgePathBuilderPass1::spacers_turn_minimize` in [`edge_path_builder_pass_1.rs`](crate/input_ir_rt/src/taffy_to_svg_elements_mapper/edge_path_builder_pass_1.rs) removes this: for a straight rank-aligned flow (both faces on the same axis) with two or more spacers, it iteratively drops any spacer that is at or inside **both** its (currently kept) neighbours' cross-axis coordinates -- and strictly inside at least one -- snapping the dropped spacer's cross-axis coordinate to the next kept turn (or the to-node contact) so its leg merges with that turn's leg. Clean monotonic staircases (no spacer inside both neighbours) are left byte-for-byte unchanged. This is a *general* notch-elimination pass over whichever spacers the curvature-specific builder receives -- it is not limited to text-content spacers, and applies equally to a cross-container spacer that happens to be less extreme than its neighbours.

    Because it depends on the resolved `start`/`end` contact points and faces -- not available where spacers are merged in `SpacerCoordinatesResolver::resolve` -- it runs later, once per curvature-specific path builder: `EdgePathBuilderPass2Ortho::build_spacer_edge_path` and `EdgePathBuilderPass2Curve::build_spacer_edge_path` both call it on the spacer slice they receive before building waypoints/curve segments. Both builders call it identically, so a notch is removed the same way regardless of `EdgeCurvature`. (In `0062_edges_from_higher_rank_to_lower_rank`, `t_aws_az1`'s own text-content spacer sits inside the wider column its cross-container spacers establish further along the path; this pass removes the resulting dip for both `Orthogonal` and `Curved` edges.)
