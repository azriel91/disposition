...
- **Label-based offset**: when an edge has a non-zero description label node on a face (looked up from `TaffyNodeMappings::edge_label_taffy_nodes`), the contact point is routed to the entry-side edge of the label -- the side the path arrives at first. Because sibling insertion order is reversed for reversed rank directions (see [Sibling order for reversed rank directions](#sibling-order-for-reversed-rank-directions)), the entry side is the same for all rank directions:
  ...
  - `EdgeContactPointOffsets` records, per slot, whether the offset is label-based (`fn is_label_based`). `EdgePathBuilderPass2::build` reads this per endpoint (via `EdgeFaceOffset::from_offset_is_label`/`to_offset_is_label`) and skips the bidirectional pair offset (see below) on that endpoint when true -- stacking both would push the contact past the node's own face bounds, since the label offset alone already separates a symmetric pair's two contacts.
...
- **Bidirectional pair offset**: `kind: symmetric` interactions/dependencies produce a forward (`EdgeType::PairRequest`) and reverse (`EdgeType::PairResponse`) edge pair. When an endpoint's offset is **not** label-based, `EdgePathBuilderPass2::build` shifts that endpoint by `node.width` (or `height_collapsed` for `Left`/`Right` faces) `* BIDIRECTIONAL_OFFSET_RATIO (0.1)`, in a direction fixed by `PairRequest`/`PairResponse`, so the two arrows of an unlabeled pair don't land on the exact same face-midpoint line.

- **Face contact clamp**: after the label/slot offset and bidirectional offset are applied, `EdgePathBuilderPass1::face_contact_clamp` clamps the contact point to the node's own face span (leaving `FACE_CONTACT_MARGIN_PX` on each side), as a defensive backstop against any offset source placing the contact outside the node's own rectangle. This runs before the curvature-specific branch, so it applies to `Orthogonal`, `Curved`, and `Direct*` edges alike.
