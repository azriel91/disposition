...
//!
//! All helpers are grouped under the [`KeyboardNav`] unit struct so that
//! call sites read as `KeyboardNav::field_keydown(...)`,
//! `KeyboardNav::card_keydown(...)`, etc.
//!
...
//! ## Focus After Remove
//!
//! [`KeyboardNav::js_focus_after_field_remove`] provides a unified strategy
//! for restoring focus when an editor field (`IdValueRow`, `*Card`,
//! `StyleAliasesSection`) is removed via **Ctrl+Shift+K**. It walks up from
//! the active element to the nearest `[data-input-diagram-field]` ancestor,
//! then tries, in order:
//!
...
/// Groups all shared keyboard-navigation helpers under a single namespace.
///
/// Call sites read as `KeyboardNav::field_keydown(...)`,
/// `KeyboardNav::card_keydown(...)`, `KeyboardNav::js_focus_first_field()`,
/// etc.
...
impl KeyboardNav {
    ...
    ///
    /// ```rust,ignore
    /// let js = KeyboardNav::js_focus_parent_entry("data-edge-group-card");
    ...
    /// - **Shift+Tab**: cycle to the previous focusable field (wraps from first
    ///   to last).
    /// - **Enter**: stop propagation so the wrapper-level handler does not
    ///   fire.
    /// - **Arrow keys**: stop propagation (allows cursor movement / native
    ///   select navigation).
    ...
    pub fn field_keydown(evt: dioxus::events::KeyboardEvent, data_attr: &str) {
        ...
        match evt.key() {
            Key::Escape => {
                ...
                document::eval(&Self::js_focus_parent_entry(data_attr));
            }
            Key::Tab => {
                evt.prevent_default();
                evt.stop_propagation();
                if shift {
                    document::eval(&Self::js_tab_prev_field(data_attr));
                } else {
                    document::eval(&Self::js_tab_next_field(data_attr));
                }
            }
            Key::Enter => {
                evt.stop_propagation();
    ...
    pub fn card_keydown(evt: dioxus::events::KeyboardEvent, data_attr: &str) -> CardKeyAction {
        ...
        match evt.key() {
            ...
            Key::ArrowUp if ctrl => {
                evt.prevent_default();
                evt.stop_propagation();
                document::eval(&Self::js_focus_first_entry(data_attr));
                CardKeyAction::None
            }
            Key::ArrowDown if ctrl => {
                evt.prevent_default();
                evt.stop_propagation();
                document::eval(&Self::js_focus_last_entry(data_attr));
                CardKeyAction::None
            }
            Key::ArrowUp => {
                evt.prevent_default();
                evt.stop_propagation();
                document::eval(&Self::js_focus_prev_entry(data_attr));
                CardKeyAction::None
            }
            Key::ArrowDown => {
                evt.prevent_default();
                evt.stop_propagation();
                document::eval(&Self::js_focus_next_entry(data_attr));
                CardKeyAction::None
            }
            Key::ArrowLeft => {
                evt.prevent_default();
            ...
            Key::Enter => {
                evt.prevent_default();
                evt.stop_propagation();
                document::eval(&Self::js_focus_first_field());
            ...
            Key::Escape => {
                ...
                document::eval(Self::JS_FOCUS_PARENT_SECTION);
                CardKeyAction::None
            }
