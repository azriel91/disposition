...
// === Thing Descriptions sub-page === //

/// The **Things: Descriptions** editor sub-page.
///
/// Edits `thing_descs` -- descriptions rendered next to things in the
/// diagram.
#[component]
pub fn ThingDescsPage(input_diagram: Signal<InputDiagram<'static>>) -> Element {
    ...
    rsx! {
        div {
            ...
            h3 { class: SECTION_HEADING, "Thing Descriptions" }
            p {
                class: "text-xs text-gray-500 mb-1",
                "Descriptions rendered next to things in the diagram."
            }

            ReorderableContainer {
                data_attr: "data-entry-id".to_owned(),
                section_id: "thing_descs".to_owned(),
                ...
                for (idx, (id, desc)) in desc_entries.iter().enumerate() {
                    {
                        let id = id.clone();
                        let desc = desc.clone();
                        let on_change = OnChangeTarget::ThingDesc;
                        ...
                        rsx! {
                            IdValueRowTextMulti {
                                ...
                                on_add: move |insert_at: usize| {
                                    EntityPageOps::thing_desc_add(&mut input_diagram.write());
