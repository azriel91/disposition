//! Container component for thing hierarchy rows.
//!
//! Wraps the list of [`ThingHierarchyRow`]s in a focusable container that
//! supports keyboard navigation:
//!
//! - **Tab**: focus the container itself (via the normal tab order).
//! - **Enter** (on container): focus the first `ThingHierarchyRow` inside.
//! - **Esc** (on a `ThingHierarchyRow`): return focus to the container.
//!
...
//!
//! [`ThingHierarchyRow`]: super::thing_hierarchy_row::ThingHierarchyRow
...
/// A focusable container for [`ThingHierarchyRow`]s.
///
/// # Props
///
...
///   focus is applied.
/// * `children`: the `ThingHierarchyRow` elements rendered inside the
///   container.
///
/// [`ThingHierarchyRow`]: super::thing_hierarchy_row::ThingHierarchyRow
#[component]
pub fn ThingHierarchyRows(mut focus_index: Signal<Option<usize>>, children: Element) -> Element {
